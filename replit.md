...
- **Session-aware auth**: Cached userId returned instantly; `getSession()` only when cache empty
- **Foreground session refresh**: AppState listener only after 10+ min AND token within 5 min of expiry
- **TOKEN_REFRESHED optimization**: Auth state change handler no longer queries profiles table on token refresh—just updates cached userId
- **profileLoadedRef**: Prevents double profile load on login (both login function and onAuthStateChange would fire)
- **Auth-aware TrackingProvider**: Watches `isAuthenticated`/`user.id` — reloads data after login, clears on logout
- **Memory caps**: In-memory GPS points capped at 10,000 (oldest trimmed). AsyncStorage background points capped at 3,000. Trip backup capped at 5,000
- **Throttled React state updates**: GPS points accumulate in ref; React state (`setTripPoints`) only updates every 10 seconds instead of every GPS tick
- **Roads API downsampling**: Before sending to Google Roads API, points downsampled to max 500 (evenly spaced)
- **Roads API total timeout**: 30-second global timeout for all snap-to-roads calls; falls back to raw GPS polyline if exceeded
- **Route points batch size**: 500 per insert (reduced from 50), reducing Supabase calls from ~67 to ~7 for a 50km trip
- **Backup interval**: Trip points backup to AsyncStorage every 60s (halves storage writes)
- **Parallel data fetching**: Summary screen fetches all date range records in parallel via `Promise.all`
